...
// CRW2-A2 / A-2: v4's formally decided execution policy is
...
test("CRW2-A2 end-to-end: run() against the real dataset-v4.json with a deliberately regressed baseline copy still exits 0 (INFORMATIONAL)", () => {
  const mutatedBaseline = loadRealBaselineV4();
  ...
  const tmpPath = path.join(require("node:os").tmpdir(), `crw2-a2-regressed-baseline-v4-${process.pid}.json`);
  fs.writeFileSync(tmpPath, JSON.stringify(mutatedBaseline));
  try {
    const result = run(DATASET_V4_PATH, tmpPath);
