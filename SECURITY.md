...
- **`browserCorrelation`** - deterministic, code-computed cross-browser metadata (which browsers failed/passed, whether failures share a signature).
...
`collect-context.js`'s `buildRelevantFiles()` applies, in order:

- **Path allowlist**: only files under `cypress/`, plus exactly `cypress.config.js` and `package.json`.
- **Filename/path denylist**: any path matching `/(^|[\\/])\.env|secret|credential|\.pem$|\.key$|token/i` is rejected, even if it would otherwise be under an allowed directory.
...
- `ai-report` (when any browser failed) - the final `context.json`/`history.json`/`ai-report.json` for the selected primary failure
...
### Future Playwright production enablement

`scripts/ai/adapters/playwright-adapter.js` is offline-only today - not wired into `collect-context.js`'s production entry point. Before any production Playwright enablement, security/privacy review must revisit the known Roadmap #19 residuals: `relevantFiles`'s Cypress-only source policy (a Playwright failure would currently reach the model with no page-object/spec context), out-of-root/absolute path handling (unsanitized today), attachment locality/materialization assumptions, and real (non-fixture) Playwright JSON-reporter compatibility. This is a pointer to that future work, not a restart of it.
...
| #20D - This governance documentation | IN PROGRESS (docs-only, pending PR merge) |

This document consolidates controls already implemented and independently verified by #20A-#20C. It does not add DLP, PII detection, new secret scanning, provider-retention enforcement, or new prompt-injection controls - see [§16](#16-known-limitations--non-goals) for what remains explicitly out of scope.
