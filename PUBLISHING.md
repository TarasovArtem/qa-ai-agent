...
|---|---|---|---|---|
| `request.testDesigns` read more than once (validated once, then re-read for canonicalization), letting a getter-/Proxy-backed property diverge between the two reads and bypass the batch-size bound and per-artifact/duplicate checks | Generic core | **Closed by RTIA-C1** | — | Found by the RTI Integrated Audit (`RTIA-B01`) via an adversarial getter-backed `request.testDesigns` probe that reproducibly exceeded `MAX_PUBLISH_BATCH_SIZE` and delivered never-validated content to `destination.publish()`. Fixed by reading `request.testDesigns` exactly once (`assertValidPublishRequest` now returns the validated array reference, reused for canonicalization); regression-tested with getter, Proxy, content-divergence, and batch-growth adversarial cases. |
| `rawResult.allSucceeded` read via ordinary property access before Boundary-B hardening fully applies | Generic core | **Closed — was already accurate as implemented, this row was stale documentation** | — | Independently re-verified during the RTI Integrated Audit (`RTIA-I01`): production code already reads `allSucceeded` via `getOwnEnumerableDataProperty` (a getter-backed value is correctly rejected as not-a-data-property, never invoked) and additionally cross-validates it against `derivedAllSucceeded` computed from actual item statuses, rejecting any destination-claimed value that disagrees. This row previously overstated an open gap; corrected rather than left to silently persist. |
| Array-index accessor hardening on untrusted `items[]`/`expectedResults[]` (a hostile numeric-index getter is not fully guarded by `Array.prototype.forEach`/spread) | Generic core (shared with RTI-1's `labels`, RTI-6's provider-array handling) | Defer to full audit | Full Project Strict Audit | Repo-wide, pre-existing pattern, not RTI-8-specific. Reconsidered and re-affirmed defer-safe by the RTI Integrated Audit; next reconsideration at Full Project Strict Audit. |
| `FAILED` result item with a known `remoteId` (a destination could theoretically report both) has no dedicated canonical representation | Generic core | Defer | Full Project Strict Audit | No current destination produces this shape; theoretical only. Reconsidered and re-affirmed defer-safe by the RTI Integrated Audit; next reconsideration at Full Project Strict Audit. |
...
| Unknown-outcome reconciliation (no mechanism to later confirm whether an `OUTCOME_UNKNOWN` item actually succeeded remotely) | Generic core / destinations | Defer | Full Project Strict Audit | Would require a durable, destination-specific "does X already exist" query — out of scope for `CREATE_ONLY` v1. Reconsidered and re-affirmed defer-safe by the RTI Integrated Audit; next reconsideration at Full Project Strict Audit. |
