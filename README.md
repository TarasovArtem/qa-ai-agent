...
**Status: RTI-1, RTI-2, RTI-3, RTI-4, RTI-5, RTI-6, and RTI-7B (Jira) complete on `main`. RTI-7F (Azure DevOps Requirements Provider) implemented, awaiting independent review (not merged). ID-3 planning remains complete but its implementation is explicitly deferred in priority behind this arc - see below.**
...
**Deliberately out of scope for this adapter**: Azure DevOps Server/TFS; tree/oneHop WIQL queries; any relation type beyond the two mapped above; a shared/generic transport abstraction with Jira (RTI-7G decides, after both adapters are on `main`, what is genuinely common - none is extracted here); a provider registry.
...
- **`ROADMAP_RTI7F_TECHNICAL_WORK`: IMPLEMENTED, AWAITING INDEPENDENT REVIEW** - `AzureDevOpsRequirementsProvider`, the second concrete `RequirementsSourceProvider`, implementing RTI-6's `{id, read()}` contract exactly with zero RTI-6/RTI-3/RTI-4/RTI-5/Jira production changes; not yet merged, not self-approved.
- **`ROADMAP_RTI7G_TECHNICAL_WORK`: NOT_STARTED** - cross-adapter architecture review (Jira vs. Azure), to run only after Azure DevOps reaches `COMPLETE_ON_MAIN`.
